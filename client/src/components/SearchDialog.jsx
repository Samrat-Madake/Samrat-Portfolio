...
const SearchDialog = ({ iconOnly = false }) => {
  ...
  if (!isOpen) {
    ...
    return (
      <button
        onClick={() => setIsOpen(true)}
        className="flex items-center justify-between gap-2 px-4 py-1.5 w-40 lg:w-48 xl:w-56 text-gray-400 hover:text-white transition-colors bg-white/10 hover:bg-white/15 rounded-lg border border-white/10 hover:border-white/20"
      >
